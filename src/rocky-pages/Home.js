...
import { loadMovieData, formatReleaseDate, getImageFilename, getImagePath, getChampionshipBelt } from '../rocky-data';
...
const Home = () => {
  ...
  const MovieCard = ({ movie }) => {
    ...
    const renderStarRating = (rating) => {
      ...
      for (let i = 1; i <= maxRating; i++) {
        if (i <= Math.floor(numericRating)) {
          // Full star
          stars.push(
            <span key={i} style={{ color: '#DAA520', fontSize: '16px', marginRight: '2px' }}>★</span>
          );
          ...
          // Empty star
          stars.push(
            <span key={i} style={{ color: '#DAA520', fontSize: '16px', marginRight: '2px', opacity: 0.3 }}>☆</span>
          );
        }
      }
    ...
    return (
      ...
      >
        <div className="rocky-card" style={{
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        padding: '60px 20px',
        background: 'linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), linear-gradient(135deg, #2C1810 0%, #8B4513 50%, #DAA520 100%)',
        position: 'relative',
        boxShadow: 'inset 0 0 100px rgba(139, 69, 19, 0.3)',
      }}>
        <style>{`
          @keyframes title-glow {
            0%, 100% { text-shadow: 0 0 20px rgba(218, 165, 32, 0.5); }
            50% { text-shadow: 0 0 40px rgba(218, 165, 32, 0.8); }
          }
        `}</style>
        <h1 style={{ 
          fontSize: '48px', 
        ...
        <p style={{ 
          ...
          Explore all {totalMovies} films from the Italian Stallion's journey - from Philadelphia to championship glory
        </p>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
            <div className="belt-champion" style={{
              width: '24px',
              height: '8px',
              borderRadius: '4px',
              border: '1px solid rgba(0, 0, 0, 0.2)'
            }}></div>
            <span style={{ fontWeight: 'bold', color: '#2C1810' }}>Heavyweight Champion</span>
            <span style={{ color: '#666' }}>(4.5+)</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
            <div className="belt-gold" style={{
              width: '24px',
              height: '8px',
              borderRadius: '4px',
              border: '1px solid rgba(0, 0, 0, 0.2)'
            }}></div>
            <span style={{ fontWeight: 'bold', color: '#2C1810' }}>Champion</span>
            <span style={{ color: '#666' }}>(4.0+)</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
            <div className="belt-platinum" style={{
              width: '24px',
              height: '8px',
              borderRadius: '4px',
              border: '1px solid rgba(0, 0, 0, 0.2)'
            }}></div>
            <span style={{ fontWeight: 'bold', color: '#2C1810' }}>Title Holder</span>
            <span style={{ color: '#666' }}>(3.5+)</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
            <div className="belt-silver" style={{
              width: '24px',
              height: '8px',
              borderRadius: '4px',
              border: '1px solid rgba(0, 0, 0, 0.2)'
            }}></div>
            <span style={{ fontWeight: 'bold', color: '#2C1810' }}>Ranked Fighter</span>
            <span style={{ color: '#666' }}>(3.0+)</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
            <div className="belt-bronze" style={{
              width: '24px',
              height: '8px',
              borderRadius: '4px',
              border: '1px solid rgba(0, 0, 0, 0.2)'
            }}></div>
            <span style={{ fontWeight: 'bold', color: '#2C1810' }}>Contender</span>
            <span style={{ color: '#666' }}>(Below 3.0)</span>
          </div>
        </div>
        <p style={{
          textAlign: 'center',
          ...
          Championship belts in the top-right corner indicate my personal rating
        </p>
      </div>
      ...
      {displayMovies.length === 0 && (
        <div style={{
          ...
          <div className="training-wisdom">
            {searchTerm ? `No results found for "${searchTerm}", champ.` : 'No movies found, champ.'}
          </div>
        </div>
      )}
    </div>
