...
const config = {
  basePath: `${process.env.PUBLIC_URL || ''}/marvel-movies`,
  csvFileName: 'movies.csv',
  movieInfoFileName: 'movieInfo.csv'
};
...
/**
 * Loads and parses the Marvel movies CSV file
...
export const loadMovieInfo = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const altPath = `/dc-movies/movieInfo.csv`;
