...
import { getMoviesByReleaseStatus, formatReleaseDate, getImageFilename, getImagePath } from '../star-wars-data';
...
function Home() {
  ...
  if (isLoading) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        <div style={{ fontSize: '24px', marginBottom: '20px' }}>Star Wars Series</div>
        <div style={{ display: 'inline-block', position: 'relative' }}>
          <div style={{
            width: '50px',
            height: '50px',
            border: '5px solid rgba(255,255,255,0.3)',
            borderTop: '5px solid #FFE81F',
            borderRadius: '50%',
            margin: '0 auto',
        ...
        <p style={{ marginTop: '20px', opacity: 0.7 }}>Loading Star Wars movies...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        <h1 style={{ fontSize: '28px', marginBottom: '20px', color: '#FFE81F' }}>Error Loading Data</h1>
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
      ...
      <div style={{
        ...
        padding: '60px 20px',
        backgroundImage: 'linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url(/star-wars/assets/sw-background.jpg)',
        backgroundSize: 'cover',
        backgroundPosition: 'center',
        position: 'relative',
        boxShadow: 'inset 0 0 100px rgba(255, 232, 31, 0.1)',
      }}>
        <style>{`
          @keyframes title-glow {
            0%, 100% { text-shadow: 0 0 20px rgba(255, 232, 31, 0.3); }
            50% { text-shadow: 0 0 40px rgba(255, 232, 31, 0.5); }
          }
        `}</style>
        <h1 style={{ 
          fontSize: '48px', 
        ...
        <p style={{ 
          ...
          Explore all {totalMovies} projects from the Star Wars saga
        </p>
          ...
          <div style={{ position: 'relative' }}>
            <style>{`
              @keyframes search-glow {
                0%, 100% { box-shadow: 0 0 10px rgba(255, 232, 31, 0.3); }
                50% { box-shadow: 0 0 20px rgba(255, 232, 31, 0.5); }
              }
              .search-input:focus {
            ...
            <input
              type="text"
              placeholder="Search for a Star Wars project..."
              value={searchTerm}
          ...
          {upcomingMovies.length > 0 && (
            <div style={{ marginBottom: '50px' }}>
              <h2 style={{ 
                ...
                color: '#FFE81F', 
                marginBottom: '20px', 
                borderBottom: '2px solid rgba(255,232,31,0.3)', 
                paddingBottom: '10px',
                display: 'flex',
                alignItems: 'center',
                gap: '10px'
...
function MovieCard({ movie, handleImageError }) {
  ...
  const renderStarRating = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <svg key={`full-${i}`} style={{ width: '16px', height: '16px', fill: '#FFE81F', marginRight: '2px' }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <svg key="half" style={{ width: '16px', height: '16px', marginRight: '2px' }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <defs>
            <linearGradient id={`halfStarGradient-${movie.Name.replace(/\s+/g, '')}`} x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="50%" stopColor="#FFE81F" />
  ...
  return (
    ...
    >
      ...
      onMouseOver={(e) => {
        ...
        e.currentTarget.style.boxShadow = '0 8px 16px rgba(255, 232, 31, 0.15), 0 0 20px rgba(255, 232, 31, 0.1)';
        e.currentTarget.style.borderColor = 'rgba(255, 232, 31, 0.5)';
      }}
      onMouseOut={(e) => {
        e.currentTarget.style.transform = 'translateY(0)';
        e.currentTarget.style.boxShadow = '0 4px 6px rgba(0,0,0,0.1)';
        e.currentTarget.style.borderColor = 'rgba(255, 232, 31, 0.2)';
      }}
      >
        {/* Poster Image */}
